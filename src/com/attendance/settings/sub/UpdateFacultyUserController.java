 ...
 */
public class UpdateFacultyUserController extends ScrollPane {
    ...
    @FXML
    private JFXButton updatefacultyuser;
    ...
    @FXML
    private TableColumn<User, String> usernameofuser;
    ...
    public UpdateFacultyUserController() {
        fxml = Fxml.getUpdateFacultyUserFXML();
        fxml.setRoot(this);
        ...
        try {
            ...
            Logger.getLogger(UpdateFacultyUserController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void initialize() {
        ...
        usernameofuser.setCellValueFactory(new PropertyValueFactory<>("username"));
        def(null);
        ...
        updatefacultyuser.setOnAction(this::update);
    }
    ...
    private void update(ActionEvent evt) {
        ...
        if (username.getText().equals(user.getUsername())) {
            ...
            if (dao.isUsernameExists(username.getText()) > 0) {
                MessageUtil.showError(Message.ERROR, "Faculty Login Account", "Username is already taken", UpdateFacultyUserController.this.getScene().getWindow());
            } else {
                newUser.setUsername(username.getText());
            }
        }
        if (changepassword.isSelected()) {
            if (newpassword.getText().equals(confirmpassword.getText())) {
                ...
                MessageUtil.showError(Message.ERROR, "Faculty Login Account", "Passwords doesn't match", UpdateFacultyUserController.this.getScene().getWindow());
            }
        } else {
        ...
        if (d) {
            MessageUtil.showInformation(Message.INFORMATION, "Faculty Login Account", "Account Updated Successfully", UpdateFacultyUserController.this.getScene().getWindow());
        } else {
            MessageUtil.showError(Message.ERROR, "Faculty Login Account", "Account Updation Failed", UpdateFacultyUserController.this.getScene().getWindow());
        }
    }
}
